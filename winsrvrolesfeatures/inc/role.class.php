<?php
/**
 * PluginWinsrvrolesfeaturesRole — Windows Server roles tab for Computer items.
 *
 * Data flow:
...
class PluginWinsrvrolesfeaturesRole extends CommonDBTM {
    ...
    public static function getTypeName($nb = 0): string {
        return _n('Windows Server Role', 'Windows Server Roles', $nb, 'winsrvrolesfeatures');
    }
    ...
    public static function showForComputer(Computer $computer): void {
        ...
        echo '<div class="mb-2 text-muted small">'
            . __('Collected by GLPI Agent during standard inventory.', 'winsrvrolesfeatures')
            . '</div>';

        if (count($rows) === 0) {
            echo '<div class="alert alert-info">'
                . __('No Windows Server roles found for this computer.', 'winsrvrolesfeatures')
                . '</div>';
            return;
        }
        ...
        echo '<th>' . __('Name', 'winsrvrolesfeatures') . '</th>';
        echo '<th>' . __('Display Name', 'winsrvrolesfeatures') . '</th>';
        echo '<th>' . __('Installed', 'winsrvrolesfeatures') . '</th>';
        echo '<th>' . __('Sub-features', 'winsrvrolesfeatures') . '</th>';
        echo '<th>' . __('Last update', 'winsrvrolesfeatures') . '</th>';
        echo '</tr></thead><tbody>';
    ...
    public function rawSearchOptions(): array {
        ...
        $options[] = [
            ...
            'field'    => 'name',
            'name'     => __('Feature name', 'winsrvrolesfeatures'),
            'datatype' => 'string',
        ];
        $options[] = [
            'id'       => 1002,
            'table'    => 'glpi_softwares',
            'field'    => 'date_mod',
            'name'     => __('Last update', 'winsrvrolesfeatures'),
            'datatype' => 'datetime',
        ];
